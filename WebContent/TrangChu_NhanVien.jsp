...
<sql:setDataSource var="snapshot" driver="com.mysql.jdbc.Driver"
	...
	and DeTai_SinhVien.TrangThai='Đề tài chờ gia han'
	 ...
	 <sql:query dataSource="${snapshot}" var="tongso">
		select count(DeTai.MaDeTai) as TongSo
       FROM DeTai
	</sql:query>
<div id="load">
...
<table class="table table-striped">
  ...
  <tr>
   <c:forEach var="row1" items="${dangky.rows}">
    ...
    <c:forEach var="row7" items="${tongso.rows}">
                   <th><c:out  value="${row7.TongSo}"/></th>
    </c:forEach>
                </tr>
              </thead>
              <tbody>
