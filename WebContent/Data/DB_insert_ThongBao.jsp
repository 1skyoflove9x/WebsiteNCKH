...
%>
        <sql:update dataSource="${snapshot}" var="result">
            INSERT INTO ThongBao(TieuDe,NoiDun,NgayDang,NguoiNhan)
			VALUES(?,?,?,?);         
            <sql:param value="${param.TieuDe}"/>
            <sql:param value="${param.NoiDung}"/>
            <sql:param value="${param.NgayDang}"/>
            <sql:param value="${param.NguoiNhan}"/>
        </sql:update>
          ...
          <c:if test="${sessionScope.quyen == 'admin'}">
            ...
            successfully.</font>
             ...
             <c:redirect url="../Actor_Admin.jsp" >
                <c:param name="errMsg" value="Thêm Thông Báo Thành Công" />
            </c:redirect> 
            </c:if>
          ...
          <c:if test="${result==0}">
            <font size="5" color='red'> False update data</font>
             ...
             <c:redirect url="../Actor_Admin.jsp" >
                <c:param name="errMsg" value="Thêm Thông Báo Thất Bại" />
            </c:redirect> 
        </c:if> 
    </c:if> 
