...
%>
        <sql:update dataSource="${snapshot}" var="result">
            UPDATE ThongTinCanBo
            SET HoTen=?,Email=?,SDT=?,DonViCongTac=?,ChucVu=?,NgaySinh=?,GioiTinh=?,CMND=?,DiaChi=?,TKNganHang=?,ChiNhanh=?
            where MaCanBo='${param.msgv}'           
            <sql:param value="${param.HoTen}"/>
            <sql:param value="${param.Email}"/>
            <sql:param value="${param.SDT}"/>
            <sql:param value="${param.DonViCongTac}"/>
              <sql:param value="${param.ChucVu}"/>
